...
\draw[style={<->,thin}] (-3.8,2.5) node[rotate=-44,font=\tiny,xshift=0.3em] {|}  -- node[below,align=center,rotate=-44] {$Slope$} (-1.15,0) node[left,yshift=-0.2em] {0} --   node[below,yshift=-0.7em,xshift=-1em,rotate=25] {$Width [nm]$} (3.75,2.2);
\node[above] at (2.8,-0.4){\includegraphics[width=0.5cm,height=3cm]{images/slopescolorbar.png}};
% \draw[step=0.25cm,gray,very thin] (0,0) grid (3.5,3);
\node[right] at (3,-0.15) {0};
\node[right] at (3,2.6) {30};
\node[below,rotate=90] at (3,1.175) {$[e^2/h]$};
\end{tikzpicture}}
  \begin{tikzpicture}[overlay]%[every text node part/.style={align=center}]
