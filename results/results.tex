...
\begin{minipage}[c]{0.5\textwidth}
 ...
 \end{flalign}
 \end{minipage}
\caption{Potential landscape of hard walled quantum point contact with triangular edges. The longitudinal direction of electron flow $x$ and the transverse direction $y$ is denoted. The depth of the wall $d$ and the width of the confinement $w$ are illustrated by the arrows. The parameter $t$ controls the extent of the triangle tip. The dimensionless analytical expression of the potential is presented on the right. The \textsc{Heaviside} step function is denoted by $\Theta$.}\label{fig:hardwalled}
\end{figure}
\begin{figure}[h!]
\begin{minipage}[c]{0.5\textwidth}
  \begin{tikzpicture}%[every text node part/.style={align=center}]
...
\includegraphics[]{images/242255-wire400x200-Dec-02-2011-23.08trans}
  \begin{tikzpicture}[overlay]%[every text node part/.style={align=center}]
      ...
      \end{tikzpicture}
\caption{Simulated transmission profile of quantum point contact of two point charges. The conductance is given in dependence of the charge of the point charges for $0 < q <7$. Note the disappearance of conductance quantization below $\approx 2\cdot10^{-18}$ C and the reappearance close to $q=0$. The corresponding potential landscape is depicted on the left.}\label{fig:qpcpointtransspin}
\end{figure}
...
\subfloat[]{\label{fig:slopes}\begin{tikzpicture}
\node[above] at (0,0){\includegraphics[width=0.46\textwidth]{images/slopesrainbow.png}};
\node[above] at (3.1,0){\includegraphics[width=0.5cm,height=3cm]{images/slopescolorbar.png}};
\draw[style={<->,thin}] (-3.5,2.2) node[left,align=center] {$m$} -- (-1.15,0) node[left,yshift=-0.2em] {0} --  (2.75,2) node[below,yshift=-0.7em,xshift=-1em] {$w$};
% \draw[step=0.25cm,gray,very thin] (0,0) grid (3.5,3);
\node[right] at (3.3,0.25) {0};
\node[right] at (3.3,3) {30};
\end{tikzpicture}}
  \begin{tikzpicture}[overlay]%[every text node part/.style={align=center}]
      \node at (-14.8,-2.3) {\includegraphics[width=0.13\textwidth]{images/qpcvariational-crop.png}};
      \end{tikzpicture}
...
\curveoutline
\end{tikzpicture}}
\caption{Electron densities of a curved nanowire of 40nm width. The wire width is marginally reduced in the curve as outined by a dashed line. Color code indicates electron density $n$. (a) First mode of energy $E=0.003$ eV. (b) Second mode of energy $E=0.036$ eV. (c) Third mode of energy $E=0.081$ eV. }
\end{figure}
\begin{figure}[h!]
