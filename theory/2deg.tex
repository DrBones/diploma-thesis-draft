A two-dimensional electron gas (2DEG) is characterized by the confinement of electron motion in one direction and free movement in the other two directions. 
...
\node at (0,-0.4){$\v{k}$};
\node at (0.7,2.2){$E_1(\v{k})$};
\node at (0.7,4.4){$E_2(\v{k})$};
\node at (0.7,6.9){$E_3(\v{k})$};
\end{tikzpicture}}
% \includegraphics[width=0.9\textwidth]{images/davies.png}
\caption{Modes in $z$-confined potential well. (a) Eigenstate densities in a potential well for energy quantum number $n$ and cut-off energies $E_1$,$E_2$,$E_3$ in dependence on the position. (b) Conductance bands in the potential well in k-space.}
