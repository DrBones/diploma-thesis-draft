...
\begin{figure}[!h]
\centering
\subfloat[]{\label{fig:discretizeddevice}\includegraphics[scale=15]{images/7x6l_leads}}
\subfloat[]{\label{fig:hamilmatrix}\includegraphics[scale=0.3]{images/rgaexample}}
\subfloat[]{\label{fig:adjacencygraph}\includegraphics[scale=0.3]{images/Spin_Graph2}}
\caption{(a) Schematic of a discretized non-colinear device. The dots denote lattice-points. The levels are indicated by dashed lines. (b) \hamil{} of the discretized nanowire. The Blocks are highlighted by dashed lines. (c) The relation between nodes are stored in the edges of the depicted graph. Nodes are represented as red circles. (not final)}
